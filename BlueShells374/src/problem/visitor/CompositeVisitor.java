...
public class CompositeVisitor implements IInvoker {
			...
			for (IClass c : m.getClasses()) {
				if (c.getAccessLevel() == 1057) {
					if (c.getInterface().size() == 1) {
						...
						this.abstractToInterface.put(c.getClassName(), inter);
					} else {
	...
	private void visitField() {
		this.visitor.addVisit(VisitType.Visit, IField.class, (ITraverser t) -> {
			...
			if (this.abstractToInterface.containsKey(this.currentClass.getExtension())
		...
		this.visitor.addVisit(VisitType.PostVisit, IModel.class, (ITraverser t) -> {
			...
			for (IClass c : m.getClasses()) {
				for (String s : this.possibleComponents) {
					...
					if ((iSize != 0 && classInterface.get(0).equals(s)) || cExt.equals(s)
						...
						if (!hasComField){
							...
							c.addPattern(new CompositePattern(c.getClassName(), "\\<\\<Leaf\\>\\>"));
						}
					}
				}
			}
			...
			for (String s : comfComp){
				for (IClass c : m.getClasses()){
					if (c.getClassName().equals(s)){
						c.addPattern(new CompositePattern(c.getClassName(), "\\<\\<Component\\>\\>"));
					}
					if (this.abstractToInterface.get(c.getClassName()) != null && this.abstractToInterface.get(c.getClassName()).equals(s)){
						c.addPattern(new CompositePattern(c.getClassName(), "\\<\\<Component\\>\\>"));
					}
				}
			}
			...
			for (IClass c2 : m.getClasses()){
				for (String s3 : comfComposite){
					if (c2.getExtension().equals(s3)){
						c2.addPattern(new CompositePattern(c2.getClassName(), "\\<\\<Composite\\>\\>"));
					}
				}
			}
		});
